...
$avail_fields = array('available', 'library', 'call_num', 'status', 'depository');
$json = array();

$url = "http://webservices.lib.harvard.edu/rest/v2/classic/holdings/$id";
...
foreach($avail->branch as $branch) {
	...
	foreach($branch->collection as $collection) {
	  ...
	  if (strpos($coll, 'Harvard Depository') !== FALSE)
	    $depository = true;
		foreach($collection->holding->items->itemrecord as $itemrecord) {
		  ...
		  if($status == 'Not checked out | Regular loan' || $status == 'Not checked out | 28-day loan'||  $status == 'Not checked out | 7-day loan' || $depository) {
		    ...
		    $thisavail = false;
		  $avail_data   = array($thisavail, $library, $call, $status, $depository);
